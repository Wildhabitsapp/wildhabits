...
// SETTINGS (xp, quickIds, affirm, quotes, refSections, walkMode, badList)
...
export async function saveSettings(uid, d) {
  ls.set('settings', d)
  try {
    await supabase.from('settings').upsert({ user_id: uid, data: d, updated_at: new Date().toISOString() })
  } catch {}
}
...
export async function saveHabit(uid, habit) {
  try {
    await supabase.from('habits').upsert({ id: habit.id, user_id: uid, data: habit, updated_at: new Date().toISOString() })
  } catch {}
}
...
export async function loadLogs(uid) {
  try {
    ...
    if (data) {
      const logs = data.map(r => ({ ts: r.ts, habitId: r.habit_id, value: r.value, note: r.note, _id: r.id, ...r.extra }))
      ls.set('logs', logs)
      return logs
    }
  } catch {}
...
export async function addLog(uid, log) {
  try {
    const { data } = await supabase.from('logs').insert({
      user_id: uid, habit_id: log.habitId, value: log.value,
      note: log.note || '', ts: log.ts,
      extra: Object.fromEntries(Object.entries(log).filter(([k]) => !['habitId','value','note','ts'].includes(k)))
    }).select().single()
    return { ...log, _id: data?.id }
  } catch { return log }
}
...
export async function loadIdeas(uid) {
  try {
    ...
    if (data) {
      const ideas = data.map(r => ({ id: r.id, text: r.text, category: r.category, priority: r.priority, done: r.done, date: r.date }))
      ls.set('ideas', ideas)
      return ideas
    }
  } catch {}
...
export async function loadSkips(uid) {
  try {
    ...
    if (data) { const s = data.map(r => ({ habitId: r.habit_id, date: r.date, reason: r.reason, ts: r.ts })); ls.set('skips', s); return s; }
  } catch {}
  return ls.get('skips', [])
}

export async function addSkip(uid, skip) {
  try { await supabase.from('skips').insert({ user_id: uid, habit_id: skip.habitId, date: skip.date, reason: skip.reason, ts: skip.ts }) } catch {}
}
...
export async function saveReflection(uid, ref) {
  try { await supabase.from('reflections').upsert({ user_id: uid, date: ref.date, data: ref.data }) } catch {}
}
